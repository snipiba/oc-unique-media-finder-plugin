...
class UniqueMediaFinder {
	...
	public function checkConfig() {
		...
		$this->isEnabled = (
			($this->providers['unsplash']->configured() && $this->providers['unsplash']->isEnabled()) || 
			($this->providers['pixabay']->configured() && $this->providers['pixabay']->isEnabled()) || 
			($this->providers['pexels']->configured() && $this->providers['pexels']->isEnabled())
		) ? true : false;
	}
	...
	public function extendMediaManager() {
		...
		$manager::extend(function($widget) {
			...
			if($this->isEnabled) {
				...
				if((bool)Settings::get('store_metadata')) {
